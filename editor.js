...
function editor_refresh_html() {
	...
	document.all('edited_html').value = f.contentWindow.document.body.innerHTML;
}

function editor_refresh_normal() {
	f = document.getElementById('editor_frame');
	f.contentWindow.document.body.innerHTML = document.all('edited_html').value;
}
...
function editor_show_html() {
	...
	document.all('edited_html').focus();
}
...
function getdown() {
	if (document.all('editor_mode_html').className == 'show0') {
		editor_refresh_html();
	}
}
...
function editor_keep_hide() {
    if (editor_timeout == 0) {
        editor_timeout = setTimeout('editor_close_submenu()', 300);
    }
}
...
function editor_insert_table() {
	...
	editor_PasteHTML('<table width='+document.all.editor_addtable_width.value+'>'+CTR+'</table>');
}
